...
async function upsertRows(table: string, rows: Record<string, unknown>[], chunkSize = 500) {
...
function importMeta(id: string): { import_source: string | null; external_id: string | null } {
  if (id.startsWith("mymoney_")) {
    return { import_source: "mymoney", external_id: id.slice("mymoney_".length) };
  }
